<#
.SYNOPSIS
  Create Summary report of Exchnage Main Box Details
.EXAMPLE
  GetMailboxStatistics.ps1
  Generate the mailbox size report with Shared mailboxes, mailbox archive.
.EXAMPLE
  GetMailboxStatistic.ps1 -sharedMailboxes only
  Get only the shared mailboxes
.EXAMPLE
  GetMailboxStatistic.ps1 -sharedMailboxes no
  Get only the user mailboxes
.EXAMPLE
  GetMailboxStatistic.ps1 -archive:$false
  Get the mailbox size without the archive mailboxes
.EXAMPLE
  GetMailboxStatistic.ps1 -CSVpath c:\users\report.csv
  Store CSV report in c:\temp\report.csv
.EXAMPLE
  GetMailboxStatistic.ps1 | Format-Table
  Print results for mailboxes in the console and format as table
.EXAMPLE
  Change Get-EXOMailbox -ResultSize unlimited ; unlimited to define number of users
#>
